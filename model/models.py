...
class PG_G(keras.Model):
    ...
    def __init__(self, latent_dim, layers):
        ...
        self.Blks = [self.create_block(block)
                     for block in range(layers)]  # 依據layer數建立 block
        self.Blks_rgb = [self.create_toRGB(
            block) for block in range(layers)]  # 依據layer數建立 rgb 層
    ...
    def create_block(self, curr_layer):
        ...
        if(curr_layer == 0):
            blk = keras.Sequential([
                keras.layers.Dense(
                    4 * 4 * 512, input_shape=(self.latent_dim,)),
...
class PG_D(keras.Model):
    ...
    def __init__(self, layers):
        ...
        self.pool = keras.layers.AvgPool2D(
    ...
    def create_block(self, curr_layer):
        ...
        if(curr_layer == self.layers - 1):
            ...
            blk = keras.Sequential([
                ...
                keras.layers.Conv2D(
                    min(self.resolution, filter*2), 3, strides=1, padding='same'),
                keras.layers.LeakyReLU(),
                keras.layers.Dropout(0.3),
            ])
        return blk
...
class PG_WGAN(keras.Model):
    ...
    def train_step(self, real_img):
        ...
        for i in range(self.d_steps):
            fake_img = self.generator(tf.random.normal(
                shape=(batchsize, self.latent_dim)))
